...
export class StorageService {
  ...
  setCookie(cname: string, cvalue: string, exdays: number, domain?: string): void {
    if (this.vars.isBrowser) {
      ...
      try {
        if (!(/^(true|false)$/.test(cvalue))) {
          newValue = JSON.parse(cvalue);
        }
      } catch (error) {
      }

      let cookieData = cvalue;
      if (newValue && !newValue?.created_date) {
        cookieData = {
          ...newValue,
          created_date: new Date().toISOString()
        };
        cookieData = encodeURIComponent(btoa(JSON.stringify(cookieData)));
      }
      const domainString = domain ? `; domain=${domain}` : '';
      document.cookie = `${cname}=${cookieData}${expires}; path=/${domainString}`;
    }
  }
